...
const MarketStoreCard = (props) => {
  ...
  return (
    <div>
      {props.market ? (
        <div
          className="card px-2 card-side bg-base-100 rounded-none border-b-2"
          onClick={() => props.setSelectedMarket()}
        >
          <div className="flex w-full">
            <figure className="m-3 rounded-lg w-24 h-24">
              <img
                className="w-full h-full"
                src={props.market.mkImg}
                alt="marketImage"
              />
            </figure>
            <div className="card-body p-3 justify-between">
              <div className="gap-0 flex flex-col">
                ...
                <span className="text-sm text-left">
                  {props.market.mkAddress}
                </span>
              </div>

              <div className="text-left text-sm">{props.market.mkPhone}</div>
            </div>
            <div className="btn mt-8 w-24 mr-4" onClick={() => navigateTo()}>
              시장 상세
            </div>
          </div>
        </div>
