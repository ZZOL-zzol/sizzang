...
const BasketPage = () => {
    ...
    const tmp = window.localStorage.getItem('basketList');
  ...
  return (
    <div className="w-full h-full bg-background-fill">
      ...
      <div className="flex flex-col w-full">
        ...
        <div className="flex flex-col items-end justify-center bg-white h-[50px] px-5 text-lg">총 100,000원</div>
      </div>
      
      <div className="fixed w-full flex flex-col bg-white bottom-0 items-center justify-center px-5 py-7 gap-2">
        {showAccount? accountList.map( account => <AccountCard account={account} setShowAccount={setShowAccount}/>) : null}     

        <button className="btn h-[40px] w-full bg-myprimary text-xl text-white" onClick={()=>onPayButtonClick()}>
